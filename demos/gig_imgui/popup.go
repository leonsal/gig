...
func NewPopupDemo() gig.IWidget {
	...
	win := gig.NewWindow("Popup").AddChildren(
		...
		gig.NewSeparator(),
		gig.NewButton("Context1").
			SetOnContext(func(w *gig.Widget) {
				popup.Open()
				fmt.Println("oncontext")
			}, imgui.HoveredFlags_None).
			SetTooltip("Right click to show context popup"),
	).SetVisible(false).SaveRef("popupDemo")
	return win
}
