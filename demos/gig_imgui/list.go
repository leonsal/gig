...
func NewListDemo() gig.IWidget {
	...
	win := gig.NewWindow("List").AddChildren(
		...
		gig.NewListBox("List1").AddStrings(
			...
			log.Println("ListBox1 changed:", l.SelectedIndex())
		}).Save(&list1),
		...
		gig.NewListBox("List2").AddItems(
			...
			SetMulti(true).
			SetOnChange(func(l *gig.ListBox) {
				log.Println("ListBox2 changed:", l.SelectedIndex())
			}).
		...
		gig.NewCombo("Combo1").AddChildren(
			...
			log.Println("Combo1 changed:", c.SelectedIndex())
		}),
